d3.parcoords = function(config) {
  ...
  function color_path_nulls(d, ctx) {
    ctx.strokeStyle = d3.functor(__.color)(d);
    ctx.beginPath();
    __.dimensions.map(function(p,i) {
      if (i == 0) {
        ...
        // null value check
        if (d[p] == "") {
          var prev = __.dimensions[i-1];
          var next = __.dimensions[i+1];
          // extend previous line slightly
          if (d[prev] !== "") {
            ctx.lineTo(position(prev)+(position(p)-position(prev))/10,yscale[prev](d[prev]));
          }

          // skip
          ctx.moveTo(position(next)-(position(next)-position(p))/10,yscale[next](d[next]));
        } else {
          ctx.lineTo(position(p),yscale[p](d[p]));
        }
      }
  ...
  function path_foreground(d) {
    return color_path_nulls(d, ctx.foreground);
  };

  function path_highlight(d) {
    return color_path_nulls(d, ctx.highlight);
  };
